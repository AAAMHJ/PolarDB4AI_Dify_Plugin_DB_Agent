...
class Polardb4aiNl2chartTool(Tool):
    ...
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage, None, None]:
        ...
        sql_by_nl2sql = tool_parameters.get("sql_by_nl2sql", "").replace("\"", "'")
        if not sql_by_nl2sql:
            raise Exception("必须填写nl2sql生成的SQL语句")
        if sql_by_nl2sql.endswith(';'):
            sql_by_nl2sql = sql_by_nl2sql[:-1]
        
        img_type = tool_parameters.get("img_t", "model")
        img_type_dict = {"model": "", "bar":"，画柱状图", "line":"，画折线图", "pie":"，画饼状图"}
        question = question + img_type_dict.get(img_type, "")
        sql = f"/*polar4ai*/SELECT * FROM PREDICT (MODEL _polar4ai_nl2chart,{sql_by_nl2sql}) WITH (usr_query = '{question}', result_type='IMAGE')"
        try:
            print(sql)
            lines = self.execute_sql(sql, tool_parameters)
            ...
            yield self.create_image_message(result)
